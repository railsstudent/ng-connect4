...
export class AlphabetaSolver implements GameSolver {
  ...
  alphabeta(currentMove: Pos, depth: number, alpha: number, beta: number, maximizingPlayer: boolean): number {
    ...
    if (depth === 0 ||
      ...
      this.iteration++;
      console.log(`[bestMove] iteration: ${this.iteration} [${currentMove.row}, ${currentMove.col}] placed ${player} depth: ${depth}:\r\n`, this.gridUtil.print());
      return x;
    }
    ...
    if (maximizingPlayer === true) {
      ...
      for (let col = 0; col < COLUMNS; col++) {
        ...
        if (this.gridUtil.canPlay(col)) {
          ...
          if (alpha >= beta) {
            console.log("[Maximized] player's turn", player, "beta cutoff", "alpha", alpha, "beta", beta, "move", move);
            //break;
            return bestScore;
          }
        }
      ...
      for (let col = 0; col < COLUMNS; col++) {
        ...
        if (this.gridUtil.canPlay(col)) {
          ...
          if (alpha >= beta) {
            console.log("[Minimized] player's turn", player, "alpha cutoff", "alpha", alpha, "beta", beta, "move", move);
            return bestScore;
          }
        }
      }
